...
Open two browser windows to `http://localhost:5173`. Enter the same room name and different player names. Use the on-screen seal buttons or **Enable camera** (local ONNX inference) to play with hands.
...
5. Video is WebRTC P2P and **never** an input path. Do not send gameplay on `{ type: "signal" }` — that is an opaque peer relay.
...
| `frontend/src/handTracker.ts` | Local YOLOX ONNX inference, labels, boxes, and game-sign mapping |
| `frontend/src/net/gameSocket.ts` | Typed join/input/reset/signal transport and input sequencing |
| `frontend/src/main.ts` | Existing UI, seal edges, state projection, camera, and WebRTC |
| `frontend/src/types.ts` | UI seal definitions and typed WebRTC signal payloads |
...
type ClientMsg =
  ...
  | { type: "signal"; payload: unknown }          // WebRTC signaling only
...
2. Update `README.md` if the public move examples should include it.
...
`frontend/src/handTracker.ts` classifies one seal at a time. `frontend/src/main.ts` converts a class change into `down`/`up` edges. **Do not** detect `TIGER, SNAKE, RAM` as a complete move in vision. **Do not** add a `gesture` message type.

### Webcam telegraph

1. `getUserMedia` + `RTCPeerConnection` are implemented. Seat `a` offers. Trickle ICE uses existing `signal` messages.
...
4. If ICE fails, seal-button and webcam gameplay continue over WebSocket. TURN is not configured.
...
TURN, accounts, anti-cheat, a larger jutsu catalog, audio, and rollback netcode. Do not leave stub files that pretend these exist.
